...
@endif
<h3 class="mb-4"><i class="fas fa-inbox"></i> Pengumpulan UAS</h3>
...
<td>{{ $item->uas->matakuliah->nama_matkul ?? '-' }}</td>
<td>
@if($item->link_pengumpulan_uas)
@if(str_starts_with($item->link_pengumpulan_uas, 'http'))
<a href="{{ $item->link_pengumpulan_uas }}" target="_blank" class="btn btn-sm btn-info">
<i class="fab fa-google-drive"></i> Lihat
</a>
@else
<a href="{{ asset('storage/' . $item->link_pengumpulan_uas) }}" target="_blank" class="btn btn-sm btn-info">
...
<td>
@if($item->nilai_uas > 0)
<span class="badge bg-success">{{ $item->nilai_uas }}</span>
@else
<span class="badge bg-warning text-dark">Belum</span>
@endif
</td>
<td>
<button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#modalNilai{{ $item->id_up_uas }}">
...
@empty
<tr>
<td colspan="7" class="text-center py-4 text-muted">Belum ada pengumpulan UAS</td>
</tr>
@endforelse
</tbody>
</table>
...
<div class="modal fade" id="modalNilai{{ $item->id_up_uas }}" tabindex="-1">
<div class="modal-dialog">
<form action="{{ route('dosen.pengumpulan.uas.update', $item->id_up_uas) }}" method="POST">
@csrf @method('PUT')
<div class="modal-content">
<div class="modal-header bg-warning">
<h5 class="modal-title"><i class="fas fa-star"></i> Beri Nilai UAS</h5>
<button type="button" class="btn-close" data-bs-dismiss="modal"></button>
</div>
<div class="modal-body">
<p><strong>Mahasiswa:</strong> {{ $item->mahasiswa->nama_mahasiswa ?? 'N/A' }}</p>
<p><strong>UAS:</strong> {{ $item->uas->matakuliah->nama_matkul ?? 'N/A' }}</p>
@if($item->kritik_dan_saran)
<div class="alert alert-info">
<strong>Kritik & Saran Mahasiswa:</strong><br>
{{ $item->kritik_dan_saran }}
</div>
@endif
<div class="mb-3">
<label class="form-label">Nilai (0-100)</label>
<input type="number" name="nilai_uas" class="form-control" value="{{ $item->nilai_uas }}" min="0" max="100" required>
</div>
<div class="mb-3">
