...
@endif
<h3 class="mb-4"><i class="fas fa-inbox"></i> Pengumpulan Tugas</h3>
...
<td>{{ $item->tugas->matakuliah->nama_matkul ?? '-' }}</td>
<td>
@if($item->link_pengumpulan_tugas)
@if(str_starts_with($item->link_pengumpulan_tugas, 'http'))
<a href="{{ $item->link_pengumpulan_tugas }}" target="_blank" class="btn btn-sm btn-info">
<i class="fab fa-google-drive"></i> Lihat
</a>
@else
<a href="{{ asset('storage/' . $item->link_pengumpulan_tugas) }}" target="_blank" class="btn btn-sm btn-info">
...
<span class="badge bg-success">{{ $item->nilai_tugas }}</span>
@else
<span class="badge bg-warning text-dark">Belum</span>
@endif
</td>
<td>
<button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#modalNilai{{ $item->id_up_tugas }}">
...
@empty
<tr>
<td colspan="8" class="text-center py-4 text-muted">Belum ada pengumpulan tugas</td>
</tr>
@endforelse
</tbody>
</table>
...
<div class="modal fade" id="modalNilai{{ $item->id_up_tugas }}" tabindex="-1">
<div class="modal-dialog">
<form action="{{ route('dosen.pengumpulan.tugas.update', $item->id_up_tugas) }}" method="POST">
@csrf @method('PUT')
<div class="modal-content">
<div class="modal-header bg-warning">
<h5 class="modal-title"><i class="fas fa-star"></i> Beri Nilai</h5>
<button type="button" class="btn-close" data-bs-dismiss="modal"></button>
</div>
<div class="modal-body">
<p><strong>Mahasiswa:</strong> {{ $item->mahasiswa->nama_mahasiswa ?? 'N/A' }}</p>
<p><strong>Tugas:</strong> {{ $item->tugas->judul ?? 'N/A' }}</p>
<div class="mb-3">
<label class="form-label">Nilai (0-100)</label>
<input type="number" name="nilai_tugas" class="form-control" value="{{ $item->nilai_tugas }}" min="0" max="100" required>
</div>
<div class="mb-3">
